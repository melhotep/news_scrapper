            ...
            try {
                const articleElements = await page.evaluate((pageUrl) => {
                    ...
                    for (const element of combinedElements) {
                        ...
                        if (title && link) {
                            ...
                            const overallConfidence = (titleConfidence + linkConfidence + 
                                                      (dateConfidence * 0.5) + 
                                                      (summaryConfidence * 0.5)) / 3;
                    ...
                    return uniqueResults
                        ...
                        .filter(result => {
                            ...
                            return result.confidence && result.confidence.overall > 0.5;
                        })
                        .sort((a, b) => {
                            // Defensive sort that handles undefined confidence
                            const aConfidence = a.confidence && a.confidence.overall ? a.confidence.overall : 0;
                            const bConfidence = b.confidence && b.confidence.overall ? b.confidence.overall : 0;
                ...
                if (request.url.includes('alarabiya.net')) {
                    try {
                        ...
                        log.error(`Error during alarabiya.net direct extraction: ${directError.message}`);
